...
class chainup(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(chainup, self).describe(), {
            ...
            'urls': {
                'test': {
                    'public': 'http://openapi.chaindown.com',
                    'private': 'http://openapi.chaindown.com',
                },
            ...
            'options': {
                ...
                'fetchMarketsMethod': 'publicGetApiCommonSymbols',
                'fetchBalanceMethod': 'privateGetAccountAccountsIdBalance',
                'createOrderMethod': 'privatePostApiCreateOrder',
                'language': 'en-US',
            },
            'commonCurrencies': {
    ...
    def parse_ticker(self, ticker, market=None):
        ...
        #                 "high": 0.02441800,
        #                 "vol": 487.70000000,
        #                 "last": 0.0242050000000000,
        #                 "low": 0.02401700,
        #                 "buy": "0.024214",
        #                 "sell": "0.024209"
        #             }
        #         ],
        #         "timestamp": 1590985871397
        #     }
        # }
        #
        symbol = None
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        ...
        response = self.publicGetApiMarketDept(self.extend(request, params))
        #
        #     {
        #     "code": "0",
        #     "msg": "suc",
        ...
        if 'tick' in data:
            ...
            timestamp = self.safe_integer(tick, 'time', self.safe_integer(data, 'time'))
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetApiGetTicker(self.extend(request, params))
        #
        #    {
        #     "code": "0",
        #     "msg": "suc",
    ...
    def fetch_tickers(self, symbols=None, params={}):
        self.load_markets()
        response = self.publicGetV1Tickers(params)
        data = self.safe_value(response, 'data')
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        request = {
            'page': 1,
            'endDate': self.ymdhms(int(round(time.time() * 1000))),
        }
        if symbol is not None:
            market = self.market(symbol)
            request['symbol'] = market['id']
        if limit is not None:
            request['pageSize'] = limit  # 1-100 orders, default is 100
        if since is not None:
            request['startDate'] = self.ymdhms(since)  # maximum query window size is 2 days, query window shift should be within past 120 days
        response = self.privateGetApiAllTrade(self.extend(request, params))
        data = self.safe_value(response, 'data')
        trades = self.parse_trades(data['resultList'], market, since, limit)
        return trades
    ...
    def fetch_trades(self, symbol, since=None, limit=1000, params={}):
        ...
        if limit is not None:
            request['size'] = limit
        response = self.publicGetV1Trades(self.extend(request, params))
        # {
        #     "code": "0",
        #     "msg": "suc",
        #     "data": [
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=1000, params={}):
        ...
        response = self.publicGetV1Kline(self.extend(request, params))
        return self.parse_ohlcvs(response['data'], market, timeframe, since, limit)
        # return response['data']

    def fetch_accounts(self, params={}):
        self.load_markets()
        response = self.privateGetApiUserAccount(params)
        return response['data']['coin_list']
    ...
    def fetch_order(self, id, symbol=None, params={}):
        ...
        response = self.privateGetApiOrderInfo(self.extend(request, params))
        data = self.safe_value(response, 'data')
        order = self.safe_value(data, 'order_info')
        return self.parse_order(order, market=market)
    ...
    def parse_order(self, order, market=None):
        #
        #             "side": "SELL",
        #             "total_price": "0.00731380",
        #             "created_at": 1588053840000,
        #             "avg_price": "0.02537157",
        #             "countCoin": "BTC",
        #             "source": 3,
        #             "type": 1,
        #             "side_msg": "卖出",
        ...
        type = None
        if ('type' in order) and (1 == order['type']):
            type = 'limit'
        if ('type' in order) and (2 == order['type']):
            type = 'market'
        status = None
        if 'status' in order:
        ...
        if (type == 'market') and (side == 'buy'):
            if self.options['createMarketBuyOrderRequiresPrice']:
                if price is None:
                    ...
                    request['volume'] = self.cost_to_precision(symbol, float(amount) * float(price))
            else:
    ...
    def cancel_order(self, id, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostApiCancelOrder({'order_id': id, 'symbol': market['id']})
        #
        #     response = {
        #         'status': 'ok',
        #         'data': '10138899000',
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private' or api == 'v2Private':
            ...
            for key in sorted(request):
                auth = auth + str(key)
                auth = auth + str(request[key])
            auth = auth + self.secret
            sign = hashlib.md5(auth.encode(encoding='UTF-8')).hexdigest()
            auth = self.urlencode(request)
            request['sign'] = sign
            auth += '&' + self.urlencode({'sign': sign})
            url += '?' + auth
            if method == 'POST':
